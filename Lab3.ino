    ...
    case 1:    //Robot Run after 3 seconds
      {
        if (bt_3_S_Time_Up)
        {
          ...
          switch (turn)
          {
            case 0://reversing code
              {
                ui_Left_Motor_Speed = constrain(ui_Motors_Speed + ui_Left_Motor_Offset, 1600, 2100) - 200;
                ui_Right_Motor_Speed = constrain(ui_Motors_Speed + ui_Right_Motor_Offset, 1600, 2100) - 200;

                if ((ui_Left_Line_Tracker_Data < (ui_Left_Line_Tracker_Dark - ui_Line_Tracker_Tolerance)) &&  (ui_Middle_Line_Tracker_Data < (ui_Middle_Line_Tracker_Dark - ui_Line_Tracker_Tolerance))  &&  (ui_Right_Line_Tracker_Data < (ui_Right_Line_Tracker_Dark - ui_Line_Tracker_Tolerance)))//if all == HIGH
                {
                  servo_LeftMotor.writeMicroseconds(ci_Left_Motor_Stop);
                  servo_RightMotor.writeMicroseconds(ci_Right_Motor_Stop);
                  ui_Left_Motor_Speed = 1700;
                  ui_Right_Motor_Speed = 1350;//--------------------------------CHECK IF THIS WORKS BETTER WITH 1300 ->1500 IS STOP------------------
...

                  {
                    ...
                    {
                      //as soon as the middle sensor is on the track, stop spinning
                      servo_LeftMotor.writeMicroseconds(ci_Left_Motor_Stop);
                      servo_RightMotor.writeMicroseconds(ci_Right_Motor_Stop);
                ...
                else if ((ui_Left_Line_Tracker_Data < (ui_Left_Line_Tracker_Dark - ui_Line_Tracker_Tolerance)) && (ui_Middle_Line_Tracker_Data < (ui_Middle_Line_Tracker_Dark - ui_Line_Tracker_Tolerance)))// if left and mid are high
                {
                  ui_Left_Motor_Speed -= 150;
                  ui_Right_Motor_Speed += 100;
                }
                else if ((ui_Right_Line_Tracker_Data < (ui_Right_Line_Tracker_Dark - ui_Line_Tracker_Tolerance)) && (ui_Middle_Line_Tracker_Data < (ui_Middle_Line_Tracker_Dark - ui_Line_Tracker_Tolerance)))// if right and mid are high
                {
                  ui_Left_Motor_Speed += 100;
                  ui_Right_Motor_Speed -= 150;
                }
                else if (ui_Right_Line_Tracker_Data < (ui_Right_Line_Tracker_Dark - ui_Line_Tracker_Tolerance))//if right==HIGH
                {
                  ui_Left_Motor_Speed += 50;
                  ui_Right_Motor_Speed = 0;
                }
                else if (ui_Left_Line_Tracker_Data < (ui_Left_Line_Tracker_Dark - ui_Line_Tracker_Tolerance))//if left==HIGH
                {
                  ui_Left_Motor_Speed = 0;
                  ui_Right_Motor_Speed += 50;//TEST IF IT WORKS SMOOTHER WITHOUT THE ADDITIONAL SPEED
                }

                else if (ui_Middle_Line_Tracker_Data < (ui_Middle_Line_Tracker_Dark - ui_Line_Tracker_Tolerance))//if mid==HIGH
                {
                  ui_Left_Motor_Speed = constrain(ui_Motors_Speed + ui_Left_Motor_Offset, 1600, 2100) - 200;
                  ui_Right_Motor_Speed = constrain(ui_Motors_Speed + ui_Right_Motor_Offset, 1600, 2100) - 200;
                }
            ...
            case 1://find the line going forward, stop and go to linetrack() when any sensor finds track
              {
                ui_Left_Motor_Speed = constrain(ui_Motors_Speed + ui_Left_Motor_Offset, 1600, 2100) - 200;
                ui_Right_Motor_Speed = constrain(ui_Motors_Speed + ui_Right_Motor_Offset, 1600, 2100) - 200;
            ...
            /*case 2://crawl mode
              {
                ui_Left_Motor_Speed = constrain(ui_Motors_Speed + ui_Left_Motor_Offset, 1600, 2100) - 250;
                ui_Right_Motor_Speed = constrain(ui_Motors_Speed + ui_Right_Motor_Offset, 1600, 2100) - 250;

                if ((ui_Left_Line_Tracker_Data < (ui_Left_Line_Tracker_Dark - ui_Line_Tracker_Tolerance)) ||  (ui_Middle_Line_Tracker_Data < (ui_Middle_Line_Tracker_Dark - ui_Line_Tracker_Tolerance))  ||  (ui_Right_Line_Tracker_Data < (ui_Right_Line_Tracker_Dark - ui_Line_Tracker_Tolerance)))//if any == HIGH
                {
                  servo_LeftMotor.writeMicroseconds(ci_Left_Motor_Stop);
                  servo_RightMotor.writeMicroseconds(ci_Right_Motor_Stop);
                  ui_Left_Motor_Speed = 1500;
            ...
            case 2: //follow the line until it ends, spin a left 90
              {
                followLine();
                if (!((ui_Left_Line_Tracker_Data < (ui_Left_Line_Tracker_Dark - ui_Line_Tracker_Tolerance)) &&  (ui_Middle_Line_Tracker_Data < (ui_Middle_Line_Tracker_Dark - ui_Line_Tracker_Tolerance))  &&  (ui_Right_Line_Tracker_Data < (ui_Right_Line_Tracker_Dark - ui_Line_Tracker_Tolerance))))//if all == LOW
                {
                  ...
                  ui_Right_Motor_Speed = 1800;//--------------------------------CHECK IF THIS WORKS BETTER WITH 1300 ->1500 IS STOP------------------
            ...
            case 3:
              //go forward to find new line(perpendicular) and then go forward a little more and do a right 90 onto it
              {
                followLine();
                if (!((ui_Left_Line_Tracker_Data < (ui_Left_Line_Tracker_Dark - ui_Line_Tracker_Tolerance)) &&  (ui_Middle_Line_Tracker_Data < (ui_Middle_Line_Tracker_Dark - ui_Line_Tracker_Tolerance))  &&  (ui_Right_Line_Tracker_Data < (ui_Right_Line_Tracker_Dark - ui_Line_Tracker_Tolerance))))//if all == LOW
                {
                  ui_Left_Motor_Speed = 1800;
                  ui_Right_Motor_Speed = 1400;//--------------------------------CHECK IF THIS WORKS BETTER WITH 1300 ->1500 IS STOP------------------
                  servo_LeftMotor.writeMicroseconds(ui_Left_Motor_Speed);
                  servo_RightMotor.writeMicroseconds(ui_Right_Motor_Speed);
                  delay(1000);
                  turn++;
            ...
            case 4://go to the end of the line, at the pad spin around
              {
                followLine();
                if ((ui_Left_Line_Tracker_Data < (ui_Left_Line_Tracker_Dark - ui_Line_Tracker_Tolerance)) &&  (ui_Middle_Line_Tracker_Data < (ui_Middle_Line_Tracker_Dark - ui_Line_Tracker_Tolerance))  &&  (ui_Right_Line_Tracker_Data < (ui_Right_Line_Tracker_Dark - ui_Line_Tracker_Tolerance)))//if all == HIGH
                  //STOP...DO A 180
            ...
            case 5://find the main line again by going forward along the line until it ends, then crawl forward to find main line, stop when sense it
              {
                if (!((ui_Left_Line_Tracker_Data < (ui_Left_Line_Tracker_Dark - ui_Line_Tracker_Tolerance)) &&  (ui_Middle_Line_Tracker_Data < (ui_Middle_Line_Tracker_Dark - ui_Line_Tracker_Tolerance))  &&  (ui_Right_Line_Tracker_Data < (ui_Right_Line_Tracker_Dark - ui_Line_Tracker_Tolerance))))//if all == LOW
                {
                  ui_Left_Motor_Speed = constrain(ui_Motors_Speed + ui_Left_Motor_Offset, 1600, 2100) - 270;

                  ui_Right_Motor_Speed = constrain(ui_Motors_Speed + ui_Right_Motor_Offset, 1600, 2100) - 270;
                ...
                else //if the sensors are reading the line, u can just track it dont be dum
                  followLine();

                break;
              }
            case 6://follow line back to other side
              {
                followLine();
                if (!((ui_Left_Line_Tracker_Data < (ui_Left_Line_Tracker_Dark - ui_Line_Tracker_Tolerance)) &&  (ui_Middle_Line_Tracker_Data < (ui_Middle_Line_Tracker_Dark - ui_Line_Tracker_Tolerance))  &&  (ui_Right_Line_Tracker_Data < (ui_Right_Line_Tracker_Dark - ui_Line_Tracker_Tolerance))))//if all == LOW
                {
...
void Reverse()
{
  ...
  else if ((ui_Right_Line_Tracker_Data < (ui_Right_Line_Tracker_Dark - ui_Line_Tracker_Tolerance)) && (ui_Middle_Line_Tracker_Data < (ui_Middle_Line_Tracker_Dark - ui_Line_Tracker_Tolerance)))// if right and mid are high
  {
    ...
    ui_Left_Motor_Speed -= 100; // constrain(ui_Motors_Speed + ui_Left_Motor_Offset , 1600, 2100)+ 50;
    // ui_Right_Motor_Speed += 25;//constrain(ui_Motors_Speed + ui_Right_Motor_Offset, 1600, 2100)-50;
  }
